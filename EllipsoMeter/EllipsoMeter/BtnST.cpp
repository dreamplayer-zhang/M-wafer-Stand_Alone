...
void CButtonST::SetID( short nID)
{
	m_nID = nID;
} // End of ID
		...
		if (wndUnderMouse && wndUnderMouse->m_hWnd != this->m_hWnd)
		{
			...
			if (m_bMouseOnButton == TRUE)
			{
				if (nFlags & MK_LBUTTON)
				{
					...
					::SendMessage(hWndParent, UM_MOUSEMOVE, (WPARAM)m_nID, (LPARAM)m_hWnd);
				}
...
void CButtonST::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	hWndParent = GetParent()->GetSafeHwnd();
	SetCapture();
	::SendMessage(hWndParent, UM_LBUTTONDOWN, (WPARAM)m_nID, (LPARAM)m_hWnd);
...
void CButtonST::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	hWndParent = GetParent()->GetSafeHwnd();
	SetCapture();
	::SendMessage(hWndParent, UM_LBUTTONUP, (WPARAM)m_nID, (LPARAM)m_hWnd);
